...
@st.cache_data()
def CONSULTA_API_GOOGLE(api_key_input, df_input, reg_select):
    ...
    def get_Google_type(x):
        if hasattr(x,'raw') and (x.raw['geometry']['location_type'] is not None): 
            return x.raw['geometry']['location_type']

    geolocator = GoogleV3(api_key=api_key, timeout=1)
    geolocate_column = df0['direccion_completa'].apply(geolocator.geocode)
    df0['direccion_api'] = geolocate_column.apply(get_address)
    df0['tipo_ubicacion'] = geolocate_column.apply(get_Google_type)
